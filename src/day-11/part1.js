...
const relief = level => Math.floor(level / 3)

const calcWorryLevel = (monkey, item) => relief(executeOperation(item, monkey.operation))
const getTargetMonkey = (monkey, item) => {
  const test = item % monkey.divisor === 0
  return test ? monkey.monkeyTrue : monkey.monkeyFalse
}
...
const inspectAndThrow = (monkey, items) => {
  const updatedItems = items[monkey.index].reduce((acc, i) => {
    const newItem = calcWorryLevel(monkey, i)
    const targetMonkey = getTargetMonkey(monkey, newItem)
...
const main = data => {
  ...
  console.log(monkeys)
  const [items, numberOfInspects] = [...new Array(20).keys()].reduce(
    ([items, numberOfInspects], round) => {
      console.log(items)
      return monkeys.reduce(
        ([accItems, accNumberOfInspects], m) => {
          return [inspectAndThrow(m, accItems), increment(accNumberOfInspects, m.index, accItems[m.index].length)]
        },
